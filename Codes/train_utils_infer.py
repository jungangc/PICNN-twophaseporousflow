...
def train(source_BHP, Qinj, Tmap, p0, sw0, n_iters, learning_rate, 
    ...
    for k in range(tsteps):
        ...
        model_save_path = '/scratch/user/jungangc/PICNN-2phase/PICNN-2phaseflow-constBHP-64by64-heter-TransferLearning-50stepsby2-final/Codes/model_infer/checkpoint2000_{}.pt'.format(k)

        model = PhyConvNet(num_classes=1, in_channels=2, time_sim = tsteps).cuda()
        if k==0:
            prev_model_save_path = '/scratch/user/jungangc/PICNN-2phase/PICNN-2phaseflow-constBHP-64by64-heter-TransferLearning-50stepsby2-final/Codes/model/checkpoint2000_{}.pt'.format(k)
            checkpoint  = torch.load(prev_model_save_path)
            model.load_state_dict(checkpoint['model_state_dict'])
        else:
            prev_model_save_path = '/scratch/user/jungangc/PICNN-2phase/PICNN-2phaseflow-constBHP-64by64-heter-TransferLearning-50stepsby2-final/Codes/model_infer/checkpoint2000_{}.pt'.format(k-1)
            checkpoint  = torch.load(prev_model_save_path)
            model.load_state_dict(checkpoint['model_state_dict'])
